...
- [x] **Backupy Postgresa: dodana redundantna kopia na 2TB** (2026-09-14) — **nie migracja**, `/srv/...` zostaje główną ścieżką jak było. Nowy katalog `/mnt/data2tb/backups/postgres/{shared,nc,test}`, wstępnie napełniony (rsync z `/srv`, zweryfikowane 1:1). Rola `backups` dostała nowe pole `mirror_dir` (obok `backups_dir`) — `pg_backup.sh.j2` po zrobieniu dumpu na `BACKUPS_DIR` kopiuje go też na `MIRROR_DIR` i stosuje tam tę samą retencję. Wszystkie 10 skryptów `/usr/local/bin/pg_backup_*.sh` (shared + 4× nc_* + 5× test_*) zaktualizowane ręcznie i przetestowane (dwa uruchomienia, plik pojawił się w obu miejscach). **Uwaga — ryzyko drift**: prawdziwe wartości `postgres_backup_instances` (nc_*/test_*) żyją tylko w zmiennej CI/CD `ANSIBLE_SECRETS` w GitLabie (nigdzie nie ma pliku `secrets.yml` — ani na serwerze, ani w `/opt/vps`) — `secrets.yml.example` w repo zaktualizowany o `mirror_dir`, ale **trzeba ręcznie dopisać `mirror_dir` do `ANSIBLE_SECRETS` w GitLab → Settings → CI/CD → Variables**, inaczej kolejny pełny `deploy_prod`/`dry_run` apply usunie mirrorowanie (wygeneruje skrypty bez `MIRROR_DIR`).
- [ ] Posprzątać stare zduplikowane wpisy crona (`pg_backup_nc.sh`/`pg_backup_test.sh` — legacy, duplikują `nc_default`/`test_default` na starych ścieżkach `/srv/...`, plus 2 osierocone komentarze `#Ansible: PostgreSQL backup prod/test` bez zadania pod spodem) — zablokowane w tej sesji przez klasyfikator agenta (zapis do crontab na zdalnym hoście), do zrobienia ręcznie albo z jawną zgodą
- [ ] Docelowo: dysk zewnętrzny jako trzeci cel kopii (user zapowiedział, 2026-09-14) — do dograna jak fizycznie podłączony
